...
Describe 'Build sync hashing fast path' {
    It 'skips content hashing when length and timestamp already match' {
        ...
        $src = Join-Path $srcDir 'big.bin'
        $dst = Join-Path $dstDir 'big.bin'
        Set-Content -LiteralPath $src -Value ('A' * 4096) -Encoding Ascii
        Copy-Item -LiteralPath $src -Destination $dst -Force
        $stamp = [datetime]::UtcNow.AddMinutes(-5)
        (Get-Item -LiteralPath $src).LastWriteTimeUtc = $stamp
        (Get-Item -LiteralPath $dst).LastWriteTimeUtc = $stamp
        $file = [pscustomobject]@{
            RelativePath = 'big.bin'
            Length = (Get-Item -LiteralPath $src).Length
            SourcePath = $src
        }
        Mock Get-FileHash { [pscustomobject]@{ Hash = 'unused' } }
        Sync-BuildFiles -Files @($file) -Destination $dstDir
        Should -Invoke Get-FileHash -Times 0
    }

    It 'falls back to hashing when timestamps differ' {
        $srcDir = Join-Path $TestDrive 'src2'
        $dstDir = Join-Path $TestDrive 'dst2'
        New-Item -ItemType Directory -Path $srcDir, $dstDir -Force | Out-Null
        $src = Join-Path $srcDir 'payload.wim'
        $dst = Join-Path $dstDir 'payload.wim'
        Set-Content -LiteralPath $src -Value ('A' * 4096) -Encoding Ascii
        Set-Content -LiteralPath $dst -Value ('B' * 4096) -Encoding Ascii
        (Get-Item -LiteralPath $src).LastWriteTimeUtc = [datetime]::UtcNow.AddMinutes(-10)
        (Get-Item -LiteralPath $dst).LastWriteTimeUtc = [datetime]::UtcNow
        $file = [pscustomobject]@{
            RelativePath = 'payload.wim'
            Length = (Get-Item -LiteralPath $src).Length
            SourcePath = $src
        }
        Mock Get-FileHash {
            if ($LiteralPath -eq $src) { [pscustomobject]@{ Hash = 'src' } }
            else { [pscustomobject]@{ Hash = 'dst' } }
        }
        Sync-BuildFiles -Files @($file) -Destination $dstDir
        Should -Invoke Get-FileHash -Times 2
    }
}
